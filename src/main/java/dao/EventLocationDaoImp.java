...
public class EventLocationDaoImp implements EventLocationDao {
    ...
    public static final String GET_ALL_EVENTS_JOIN_LOCATION_LIMIT = "SELECT event.id, event.name, event.description," +
            " event.cost,event.is_active,location.address,event.address_id,event.date FROM event " +
            "LEFT JOIN location ON event.address_id = location.id ORDER BY name,id LIMIT 5 OFFSET ?;";
    ...
    public List<Event> getAllEventsJoinLocation(int pageId) {
        try {
            PreparedStatement preparedStatement = ConnectionUtils.getConnection().prepareStatement(GET_ALL_EVENTS_JOIN_LOCATION_LIMIT, ResultSet.TYPE_SCROLL_SENSITIVE,
